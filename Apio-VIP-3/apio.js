    ...
    Apio.Socket.init = function (httpInstance, sessionMiddleware) {
        if (!Apio.hasOwnProperty("io")) {
            ...
            Apio.io.on("connection", function (socket) {
                ...
                socket.on("ask_connections", function () {
                    for (var xx in Apio.connectedSockets) {
                        if (xx === "admin" || validator.isEmail(xx)) {
                            for (var ii in Apio.connectedSockets[xx]) {
                                Apio.io.sockets.connected[Apio.connectedSockets[xx][ii]].emit("update_connected_socket", numberOfConnection);
                            }
                        }
                    }
                });
                ...
                var associateSocket = function (name, id) {
                    ...
                    if (!Apio.connectedSockets.hasOwnProperty(name)) {
                        ...
                        if (name === "admin" || validator.isEmail(name)) {
                            numberOfConnection++;
                        }
                    }
                ...
                socket.on("disconnect", function () {
                    ...
                    for (var i = 0, found = false; !found && i < socketKeys.length; i++) {
                        ...
                        if (Apio.connectedSockets[socketKeys[i]].length === 0) {
                            ...
                            if (socketKeys[i] === "admin" || validator.isEmail(socketKeys[i])) {
                                numberOfConnection--;
                                for (var xx in Apio.connectedSockets) {
                                    if (xx === "admin" || validator.isEmail(xx)) {
                                        for (var ii in Apio.connectedSockets[xx]) {
